...
namespace Core.Arango.Linq.Internal.Util.Extensions
{
    internal static class ExpressionExtensions
    {
        ...
        {
            if (!(expr is LambdaExpression lambda)) lambda = Lambda(expr);
        ...
        {
            return expr is UnaryExpression uexpr && uexpr.NodeType == ExpressionType.Convert ? uexpr.Operand : expr;
        }

        public static bool IsEmpty(this Expression expr)
        {
            return expr is DefaultExpression && expr.Type == typeof(void);
        }

        public static bool IsClosedVariable(this Expression expr)
        {
            return expr is MemberExpression mexpr && (mexpr.Expression?.Type.IsClosureClass() ?? false);
        }
        ...
        public static string Name(this Expression expr, string language = "C#")
        {
            ...
            {
                return (instance is null ? $"{mi.DeclaringType.FriendlyName(language)}." : "") + mi.Name;
            }
