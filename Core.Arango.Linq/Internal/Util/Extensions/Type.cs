...
namespace Core.Arango.Linq.Internal.Util.Extensions
{
    internal static class TypeExtensions
    {
        ...
        public static Type UnderlyingIfNullable(this Type type)
        {
        ...
        {
            return type.UnderlyingIfNullable().In(numericTypes);
        }
        ...
        public static bool InheritsFromOrImplementsAny(this Type type, IEnumerable<Type> types)
        {
        ...
        {
            return type != null && type.HasAttribute<CompilerGeneratedAttribute>() &&
                   type.Name.ContainsAny("DisplayClass", "Closure$");
        }

        public static bool IsAnonymous(this Type type)
        {
            return type.HasAttribute<CompilerGeneratedAttribute>() && type.Name.Contains("Anonymous") &&
                   type.Name.ContainsAny("<>", "VB$") && !type.InheritsFromOrImplements<Delegate>();
        }

        public static bool IsVBAnonymousDelegate(this Type type)
        {
            return type.HasAttribute<CompilerGeneratedAttribute>() && type.Name.Contains("VB$AnonymousDelegate");
        }
        ...
        public static string FriendlyName(this Type type, string language)
        {
            ...
            if (type.IsClosureClass()) return language == CSharp ? "<closure>" : "<Closure>";
            ...
            if (type.IsArray)
            {
                ...
                var arraySpecifiers = nestedArrayTypes.Joined("",
            ...
            if (!type.IsGenericType)
            {
                ...
                if (type.IsVBAnonymousDelegate()) return "VB$AnonymousDelegate";

                var dict = language == CSharp ? CSKeywordTypes : VBKeywordTypes; // language == VisualBasic
            ...
            var nongenericName =
                type.IsVBAnonymousDelegate() ? "VB$AnonymousDelegate" : type.Name.Substring(0, backtickIndex);
            return language == CSharp ? $"{nongenericName}<{parts}>" : $"{nongenericName}(Of {parts})";
        }
        ...
        public static Type? ItemType(this Type type)
        {
            ...
            if (ImplIEnumT(type)) return type.GetGenericArguments().First();

            // type implements/extends IEnumerable<T>;
            var enumType = type.GetInterfaces().Where(ImplIEnumT).Select(t => t.GetGenericArguments().First())
                .FirstOrDefault();
            if (enumType != null) return enumType;

            // type is IEnumerable
            if (IsIEnum(type) || type.GetInterfaces().Any(IsIEnum)) return typeof(object);

            return null;

            static bool IsIEnum(Type t)
            {
                return t == typeof(IEnumerable);
            }

            static bool ImplIEnumT(Type t)
            {
        ...
        {
            if (andSelf) yield return t;
            if (t.IsGenericType && genericDefinitions) yield return t.GetGenericTypeDefinition();

            foreach (var i in t.GetInterfaces()) yield return reduceToGeneric(i);
            if (t.BaseType != null)
