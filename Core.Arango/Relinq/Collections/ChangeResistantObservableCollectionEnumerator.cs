...
{
    /// <summary>
    ///     Provides a way to enumerate an <see cref="ObservableCollection{T}" /> while items are inserted, removed, or cleared
    ///     in a consistent fashion.
    /// </summary>
    /// <typeparam name="T">The element type of the <see cref="ObservableCollection{T}" />.</typeparam>
    /// <remarks>
    ///     This class subscribes to the <see cref="ObservableCollection{T}.CollectionChanged" /> event exposed by
            ...
            get
            {
                ...
                if (_index >= _collection.Count)
                    throw new InvalidOperationException(
                        "After MoveNext returned false, Current must not be invoked any more.");
