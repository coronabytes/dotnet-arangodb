...
{
    /// <summary>
    ///     The <see cref="IEvaluatableExpressionFilter" /> interface defines an extension point for disabling partial
    ///     evaluation on specific <see cref="Expression" /> nodes.
    /// </summary>
    /// <remarks>
    ///     <para>
    ///         Implement the individual evaluation methods and return <see langword="false" /> to mark a specfic
    ///         <see cref="Expression" /> node as not partially
    ///         evaluatable. Note that the partial evaluation infrastructure will take care of visiting an
    ///         <see cref="Expression" /> node's children,
    ///         so the determination can usually be constrained to the attributes of the <see cref="Expression" /> node itself.
    ///     </para>
    ///     <para>
    ///         Use the <see cref="EvaluatableExpressionFilterBase" /> type as a base class for filter implementations that
    ...
    internal interface IEvaluatableExpressionFilter
    {
