...
{
    /// <summary>
    ///     Dynamically discovers attributes implementing the <see cref="IMethodCallExpressionTransformerAttribute" />
    ///     interface on methods and get accessors
    ///     invoked by <see cref="MethodCallExpression" /> or <see cref="MemberExpression" /> instances and applies the
    ///     respective
    ///     <see cref="IExpressionTransformer{T}" />.
    /// </summary>
    internal class AttributeEvaluatingExpressionTransformer : IExpressionTransformer<Expression>
    {
        ...
        public Expression Transform(Expression expression)
        {
            ...
            if (memberExpression != null && memberExpression.Member is PropertyInfo)
            {
                ...
                if (methodCallExpressionTransformerProvider != null)
                    return ApplyTransformer(methodCallExpressionTransformerProvider,
                        Expression.Call(memberExpression.Expression, getter));
            }
        ...
        private static Expression ApplyTransformer(IMethodCallExpressionTransformerAttribute provider,
        ...
        /// <summary>
        ///     Defines an interface for attributes providing an <see cref="IExpressionTransformer{T}" /> for a given
        ///     <see cref="MethodCallExpression" />.
        /// </summary>
        /// <remarks>
        ///     <para>
        ///         <see cref="AttributeEvaluatingExpressionTransformer" /> detects attributes implementing this interface while
        ///         expressions are parsed
        ///         and uses the <see cref="IExpressionTransformer{T}" /> returned by <see cref="GetExpressionTransformer" /> to
        ///         modify the expressions.
        ///     </para>
        ///     <para>
        ///         Only one attribute instance implementing <see cref="IMethodCallExpressionTransformerAttribute" /> must be
