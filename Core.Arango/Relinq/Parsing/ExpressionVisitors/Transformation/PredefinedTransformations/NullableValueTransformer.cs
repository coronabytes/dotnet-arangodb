...
{
    /// <summary>
    ///     Replaces calls to <see cref="Nullable{T}.Value" /> and <see cref="Nullable{T}.HasValue" /> with casts and null
    ///     checks. This allows LINQ providers
    ///     to treat nullables like reference types.
    /// </summary>
    internal class NullableValueTransformer : IExpressionTransformer<MemberExpression>
    {
        ...
        public Expression Transform(MemberExpression expression)
        {
            ...
            if (expression.Member.Name == "HasValue" && IsDeclaredByNullableType(expression.Member))
        ...
        {
            return memberInfo.DeclaringType.GetTypeInfo().IsGenericType &&
                   memberInfo.DeclaringType.GetGenericTypeDefinition() == typeof(Nullable<>);
        }
    }
}