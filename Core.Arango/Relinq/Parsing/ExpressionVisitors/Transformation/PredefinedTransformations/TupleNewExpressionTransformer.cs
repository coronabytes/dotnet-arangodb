...
{
    /// <summary>
    ///     Detects <see cref="NewExpression" /> nodes for the .NET tuple types and adds <see cref="MemberInfo" /> metadata to
    ///     those nodes.
    ///     This allows LINQ providers to match member access and constructor arguments more easily.
    /// </summary>
    internal class TupleNewExpressionTransformer : MemberAddingNewExpressionTransformerBase
    {
        ...
        {
            return arguments
                .Select((expr, i) => GetMemberForNewExpression(constructorInfo.DeclaringType, "Item" + (i + 1)))
                .ToArray();
        }
    }
}