...
{
    /// <summary>
    ///     Replaces all nodes that equal a given <see cref="Expression" /> with a replacement node. Expressions are also
    ///     replaced within subqueries; the
    ///     <see cref="QueryModel" /> is changed by the replacement operations, it is not copied. The replacement node is not
    ///     recursively searched for
    ///     occurrences of the <see cref="Expression" /> to be replaced.
    /// </summary>
    internal sealed class ReplacingExpressionVisitor : RelinqExpressionVisitor
    {
        public static Expression Replace(Expression replacedExpression, Expression replacementExpression,
