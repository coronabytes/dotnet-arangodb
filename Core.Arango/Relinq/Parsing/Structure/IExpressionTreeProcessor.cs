...
{
    /// <summary>
    ///     <see cref="IExpressionTreeProcessor" /> is implemented by classes that represent steps in the process of parsing
    ///     the structure
    ///     of an <see cref="Expression" /> tree. <see cref="ExpressionTreeParser" /> applies a series of these steps to the
    ///     <see cref="Expression" />
    ///     tree before analyzing the query operators and creating a <see cref="QueryModel" />.
    /// </summary>
    /// <remarks>
    ///     <para>
    ///         There are predefined implementations of <see cref="IExpressionTreeProcessor" /> that should only be left out
    ///         when parsing an
    ///         <see cref="Expression" /> tree when there are very good reasons to do so.
    ///     </para>
    ///     <para>
    ///         <see cref="IExpressionTreeProcessor" /> can be implemented to provide custom, complex transformations on an
    ///         <see cref="Expression" />
    ///         tree. For performance reasons, avoid adding too many steps each of which visits the whole tree. For
    ///         simple transformations, consider using <see cref="IExpressionTransformer{T}" /> and
    ///         <see cref="TransformingExpressionTreeProcessor" /> - which can
