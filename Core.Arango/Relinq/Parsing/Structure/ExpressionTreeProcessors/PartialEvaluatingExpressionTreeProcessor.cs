...
{
    /// <summary>
    ///     Analyzes an <see cref="Expression" /> tree for sub-trees that are evaluatable in-memory, and evaluates those
    ///     sub-trees.
    /// </summary>
    /// <remarks>
    ///     The <see cref="PartialEvaluatingExpressionTreeProcessor" /> uses the
    ///     <see cref="PartialEvaluatingExpressionVisitor" /> for partial evaluation.
    ///     It performs two visiting runs over the <see cref="Expression" /> tree.
    /// </remarks>
    internal sealed class PartialEvaluatingExpressionTreeProcessor : IExpressionTreeProcessor
    {
        ...
        public Expression Process(Expression expressionTree)
        {
            ...
            return PartialEvaluatingExpressionVisitor.EvaluateIndependentSubtrees(expressionTree, Filter);
        }
    }
}