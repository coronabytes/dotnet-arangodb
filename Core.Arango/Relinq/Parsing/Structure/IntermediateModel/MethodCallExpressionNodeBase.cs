...
{
    /// <summary>
    ///     Base class for <see cref="IExpressionNode" /> implementations that represent instantiations of
    ///     <see cref="MethodCallExpression" />.
    /// </summary>
    internal abstract class MethodCallExpressionNodeBase : IExpressionNode
    {
        ...
        ///     when implementing a <see cref="IExpressionNode" /> that does not need a subquery to be created if it occurs after
        ///     the query end.
        /// </summary>
        /// <remarks>
        ///     <para>
        ...
        ///         the current query model into a <see cref="SubQueryExpression" /> that we put into the
        ///         <see cref="MainFromClause" /> of a new
        ///         <see cref="QueryModel" />.
        ///     </para>
        ///     <para>
        ///         This method also changes the <see cref="Source" /> of this node because logically, all <see cref="Resolve" />
        ///         operations must be handled
        ///         by the new <see cref="MainFromClause" /> holding the <see cref="SubQueryExpression" />. For example, consider
        ...
        ///   .Select (x => x)
        /// </code>
        ///         Naively, the last Select node would resolve (via Distinct and Select) to the <see cref="MainFromClause" />
        ///         created by the initial MainSource.
        ///         After this method is executed, however, that <see cref="MainFromClause" /> is part of the sub query, and a new
        ...
        ///     </para>
        /// </remarks>
        protected virtual QueryModel WrapQueryModelAfterEndOfQuery(QueryModel queryModel,
            ClauseGenerationContext clauseGenerationContext)
        {
            ArgumentUtility.CheckNotNull("queryModel", queryModel);
            ...
            if (sourceAsResultOperatorNode != null)
                return WrapQueryModel(queryModel, sourceAsResultOperatorNode.AssociatedIdentifier,
        ...
        /// <remarks>
        ///     By default, the result type override is set to <see cref="NodeResultType" /> in the <see cref="Apply" /> method.
        ...
        ///     <see cref="QueryModel.ResultTypeOverride" /> to another value, or to clear it (set it to <see langword="null" />).
        ///     Do not leave the
        ///     <see cref="QueryModel.ResultTypeOverride" /> unchanged when overriding this method, as a source node might have set
        ...
        private QueryModel WrapQueryModel(QueryModel queryModel, string associatedIdentifier,
            ClauseGenerationContext clauseGenerationContext)
        {
        ...
        protected NotSupportedException CreateResolveNotSupportedException()
        {
            return new(
                GetType().Name +
                " does not support resolving of expressions, because it does not stream any data to the following node.");
        }
        ...
        protected NotSupportedException CreateOutputParameterNotSupportedException()
        {
            return new(
                GetType().Name +
                " does not support creating a parameter for its output because it does not stream any data to the following node.");
        }
    }
}