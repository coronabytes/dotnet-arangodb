...
{
    /// <summary>
    ///     <see cref="TupleExpressionBuilder" /> can be used to build tuples incorporating a sequence of
    ///     <see cref="Expression" />s.
    ///     For example, given three expressions, exp1, exp2, and exp3, it will build nested <see cref="NewExpression" />s that
    ///     are equivalent to the
    ///     following: new KeyValuePair(exp1, new KeyValuePair(exp2, exp3)).
    ///     Given an <see cref="Expression" /> whose type matches that of a tuple built by
    ///     <see cref="TupleExpressionBuilder" />, the builder can also return
    ...
    ///     This class can be handy whenever a set of <see cref="Expression" /> needs to be put into a single
    ///     <see cref="Expression" />
    ...
    public static class TupleExpressionBuilder
    {
        ...
        public static IEnumerable<Expression> GetExpressionsFromTuple(Expression tupleExpression)
        {
            ...
            while (tupleExpression.Type.GetTypeInfo().IsGenericType &&
                   tupleExpression.Type.GetGenericTypeDefinition() == typeof(KeyValuePair<,>))
            {
                yield return Expression.MakeMemberAccess(tupleExpression,
                    tupleExpression.Type.GetRuntimeProperty("Key"));
                tupleExpression =
                    Expression.MakeMemberAccess(tupleExpression, tupleExpression.Type.GetRuntimeProperty("Value"));
            }
