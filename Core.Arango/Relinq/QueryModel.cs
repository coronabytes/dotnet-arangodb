...
{
    /// <summary>
    ///     Provides an abstraction of an expression tree created for a LINQ query. <see cref="QueryModel" /> instances are
    ///     passed to LINQ providers based
    ///     on re-linq via <see cref="IQueryExecutor" />, but you can also use <see cref="QueryParser" /> to parse an
    ///     expression tree by hand or construct
    ///     a <see cref="QueryModel" /> manually via its constructor.
    /// </summary>
    /// <remarks>
    ///     The different parts of the query are mapped to clauses, see <see cref="MainFromClause" />,
    ///     <see cref="BodyClauses" />, and
    ///     <see cref="SelectClause" />. The simplest way to process all the clauses belonging to a <see cref="QueryModel" />
    ///     is by implementing
    ///     <see cref="IQueryModelVisitor" /> (or deriving from <see cref="QueryModelVisitorBase" />) and calling
    ///     <see cref="Accept" />.
    /// </remarks>
    internal sealed class QueryModel
    {
        ...
        /// <summary>
        ///     Gets or sets the query's <see cref="Clauses.MainFromClause" />. This is the starting point of the query, generating
        ...
        ///     for each of the
        ///     items coming from the <see cref="MainFromClause" /> and passing the <see cref="BodyClauses" />. After it, only the
        ...
        ///     filtering (<see cref="WhereClause" />), ordering (<see cref="OrderByClause" />), augmenting (
        ///     <see cref="AdditionalFromClause" />), or otherwise
        ///     processing them before they are passed to the <see cref="SelectClause" />.
        /// </summary>
        public ObservableCollection<IBodyClause> BodyClauses { get; }

        /// <summary>
        ///     Gets the result operators attached to this <see cref="SelectClause" />. Result operators modify the query's result
        ///     set, aggregating,
        ///     filtering, or otherwise processing the result before it is returned.
        /// </summary>
        public ObservableCollection<ResultOperatorBase> ResultOperators { get; }
        ...
        /// <summary>
        ///     Gets an <see cref="IStreamedDataInfo" /> object describing the data streaming out of this <see cref="QueryModel" />
        ///     . If a query ends with
        ///     the <see cref="SelectClause" />, this corresponds to <see cref="Clauses.SelectClause.GetOutputDataInfo" />. If a
        ///     query has
        ///     <see cref="QueryModel.ResultOperators" />, the data is further modified by those operators.
        /// </summary>
        /// <returns>
        ///     Gets a <see cref="IStreamedDataInfo" /> object describing the data streaming out of this
        ///     <see cref="QueryModel" />.
        /// </returns>
        /// <remarks>
        ...
        ///     query ends with a <see cref="ResultOperatorBase" />. For example, if the query ends with a
        ///     <see cref="CountResultOperator" />, the
        ///     result type will be <see cref="int" />.
        /// </remarks>
        /// <exception cref="InvalidOperationException">
        ///     The <see cref="ResultTypeOverride" /> is not compatible with the calculated <see cref="IStreamedDataInfo" />
        ///     calculated from the <see cref="ResultOperators" />.
        /// </exception>
        ...
        /// <summary>
        ///     Accepts an implementation of <see cref="IQueryModelVisitor" /> or <see cref="QueryModelVisitorBase" />, as defined
        ...
        public override string ToString()
        {
            ...
            if (IsIdentityQuery())
                ...
                mainQueryString = MainFromClause + BodyClauses.Aggregate("", (s, b) => s + " " + b) + " " +
                                  SelectClause;
        ...
        /// <summary>
        ///     Clones this <see cref="QueryModel" />, returning a new <see cref="QueryModel" /> equivalent to this instance, but
        ///     with its clauses being
        ///     clones of this instance's clauses. Any <see cref="QuerySourceReferenceExpression" /> in the cloned clauses that
        ///     points back to another clause
        ///     in this <see cref="QueryModel" /> (including its subqueries) is adjusted to point to the respective clones in the
        ///     cloned
        ...
        ///     The <see cref="QuerySourceMapping" /> defining how to adjust instances of
        ///     <see cref="QuerySourceReferenceExpression" /> in the cloned <see cref="QueryModel" />. If there is a
        ///     <see cref="QuerySourceReferenceExpression" />
        ///     that points out of the <see cref="QueryModel" /> being cloned, specify its replacement via this parameter. At the
        ...
        /// <summary>
        ///     Transforms all the expressions in this <see cref="QueryModel" />'s clauses via the given
        ...
        ///     <see cref="QueryModel" />, and those expressions will be replaced with what the delegate returns.
        /// </param>
        ...
        /// <summary>
        ///     Returns a new name with the given prefix. The name is different from that of any <see cref="FromClauseBase" />
        ///     added
        ///     in the <see cref="QueryModel" />. Note that clause names that are changed after the clause is added as well as
        ...
        /// <summary>
        ///     Executes this <see cref="QueryModel" /> via the given <see cref="IQueryExecutor" />. By default, this indirectly
        ///     calls
        ...
        /// <summary>
        ///     Determines whether this <see cref="QueryModel" /> represents an identity query. An identity query is a query
        ///     without any body clauses
        ///     whose <see cref="SelectClause" /> selects exactly the items produced by its <see cref="MainFromClause" />. An
        ///     identity query can have
        ///     <see cref="ResultOperators" />.
        /// </summary>
        /// <returns>
        ///     <see langword="true" /> if this <see cref="QueryModel" /> represents an identity query; otherwise,
        ///     <see langword="false" />.
        /// </returns>
        /// <example>
        ///     An example for an identity query is the subquery in that is produced for the
        ///     <see cref="Clauses.SelectClause.Selector" /> in the following
        ///     query:
        ///     <code>
        /// from order in ...
        /// select order.OrderItems.Count()
        /// </code>
        ///     In this query, the <see cref="Clauses.SelectClause.Selector" /> will become a <see cref="SubQueryExpression" />
        ///     because
        ///     <see cref="Enumerable.Count{TSource}(System.Collections.Generic.IEnumerable{TSource})" /> is treated as a query
        ///     operator. The
        ///     <see cref="QueryModel" /> in that <see cref="SubQueryExpression" /> has no <see cref="BodyClauses" /> and a trivial
        ///     <see cref="SelectClause" />,
        ///     so its <see cref="IsIdentityQuery" /> method returns <see langword="true" />. The outer <see cref="QueryModel" />,
        ///     on the other hand, does not
        ///     have a trivial <see cref="SelectClause" />, so its <see cref="IsIdentityQuery" /> method returns
        ///     <see langword="false" />.
        /// </example>
        ...
        /// <summary>
        ///     Creates a new <see cref="QueryModel" /> that has this <see cref="QueryModel" /> as a sub-query in its
        ///     <see cref="MainFromClause" />.
        /// </summary>
        /// <param name="itemName">The name of the new <see cref="QueryModel" />'s <see cref="FromClauseBase.ItemName" />.</param>
        /// <returns>
        ///     A new <see cref="QueryModel" /> whose <see cref="MainFromClause" />'s <see cref="FromClauseBase.FromExpression" />
        ///     is a
        ///     <see cref="SubQueryExpression" /> that holds this <see cref="QueryModel" /> instance.
        /// </returns>
