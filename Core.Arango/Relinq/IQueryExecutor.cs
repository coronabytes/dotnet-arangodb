...
namespace Core.Arango.Relinq
{
    ...
    ///     this interface
    ///     and <see cref="QueryProviderBase" /> calls the respective method of the interface implementation when a query is to
    ///     be executed.
    /// </summary>
    internal interface IQueryExecutor
    {
        ...
        ///     The query ends with a scalar result operator, for example a <see cref="CountResultOperator" /> or a
        ///     <see cref="SumResultOperator" />.
        /// </summary>
        /// <typeparam name="T">The type of the scalar value returned by the query.</typeparam>
        /// <param name="queryModel">
        ///     The <see cref="QueryModel" /> representing the query to be executed. Analyze this via an
        ///     <see cref="IQueryModelVisitor" />.
        /// </param>
        /// <returns>A scalar value of type <typeparamref name="T" /> that represents the query's result.</returns>
        /// <remarks>
        ///     The difference between <see cref="ExecuteSingle{T}" /> and <see cref="ExecuteScalar{T}" /> is in the kind of object
        ///     that is returned.
        ///     <see cref="ExecuteSingle{T}" /> is used when a query that would otherwise return a collection result set should
        ///     pick a single value from the
        ///     set, for example the first, last, minimum, maximum, or only value in the set. <see cref="ExecuteScalar{T}" /> is
        ...
        ///     object of type
        ///     <typeparamref name="T" />.
        ///     The query ends with a single result operator, for example a <see cref="FirstResultOperator" /> or a
        ///     <see cref="SingleResultOperator" />.
        /// </summary>
        ...
        ///     if <see langword="false" />, it should throw an <see cref="InvalidOperationException" /> when its result set is
        ///     empty.
        /// </param>
        ...
        ///     <see cref="SingleResultOperator" /> or <see cref="FirstResultOperator" />. In such a case, the returned enumerable
        ///     must yield exactly
