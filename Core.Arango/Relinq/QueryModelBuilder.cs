...
{
    /// <summary>
    ///     Collects clauses and creates a <see cref="QueryModel" /> from them. This provides a simple way to first add all the
    ///     clauses and then
    ///     create the <see cref="QueryModel" /> rather than the two-step approach (first <see cref="SelectClause" /> and
    ///     <see cref="MainFromClause" />,
    ///     then the <see cref="IBodyClause" />s) required by <see cref="QueryModel" />'s constructor.
    /// </summary>
    internal sealed class QueryModelBuilder
    {
        private readonly List<IBodyClause> _bodyClauses = new();
        private readonly List<ResultOperatorBase> _resultOperators = new();

        public MainFromClause MainFromClause { get; private set; }
        public SelectClause SelectClause { get; private set; }

        public ReadOnlyCollection<IBodyClause> BodyClauses => new(_bodyClauses);

        public ReadOnlyCollection<ResultOperatorBase> ResultOperators => new(_resultOperators);
