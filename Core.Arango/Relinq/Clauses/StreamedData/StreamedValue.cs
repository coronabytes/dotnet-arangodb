...
namespace Core.Arango.Relinq.Clauses.StreamedData
{
    ...
    ///     <see cref="ResultOperatorBase.ExecuteInMemory" />.  The data is a single, non-sequence value and can only be
    ///     consumed by result operators
    ///     working with single values.
    /// </summary>
    internal sealed class StreamedValue : IStreamedData
    {
        /// <summary>
        ///     Initializes a new instance of the <see cref="StreamedValue" /> class, setting the <see cref="Value" /> and
        ...
        /// </value>
        public StreamedValueInfo DataInfo { get; }

        /// <summary>
        ///     Gets the current value for the <see cref="ResultOperatorBase.ExecuteInMemory(IStreamedData)" /> operation. If the
        ...
        /// <summary>
        ///     Gets the value held by <see cref="Value" />, throwing an exception if the value is not of type
        ///     <typeparamref name="T" />.
        /// </summary>
        ...
        public T GetTypedValue<T>()
        {
            ...
            {
                var message = string.Format("Cannot retrieve the current value as type '{0}' because it is null.",
                    typeof(T).FullName);
                throw new InvalidOperationException(message, ex);
            }
        }
