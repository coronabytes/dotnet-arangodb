...
namespace Core.Arango.Relinq.Clauses
{
    ...
    /// </summary>
    internal abstract class ResultOperatorBase
    {
        /// <summary>
        ...
        /// </summary>
        /// <param name="input">
        ///     The input for the result operator. This must match the type of <see cref="IStreamedData" />
        ...
        /// <summary>
        ///     Gets information about the data streamed out of this <see cref="ResultOperatorBase" />. This contains the result
        ///     type a query would have if
        ///     it ended with this <see cref="ResultOperatorBase" />, and it optionally includes an
        ///     <see cref="StreamedSequenceInfo.ItemExpression" /> describing
        ///     the streamed sequence's items.
        /// </summary>
        /// <param name="inputInfo">
        ///     Information about the data produced by the preceding <see cref="ResultOperatorBase" />, or the
        ///     <see cref="SelectClause" />
        ///     of the query if no previous <see cref="ResultOperatorBase" /> exists.
        /// </param>
        ...
        ///     <see cref="QueryModel.ResultOperators" /> collection.
        /// </param>
        public void Accept(IQueryModelVisitor visitor, QueryModel queryModel, int index)
        {
            ...
            if (this is AsQueryableResultOperator &&
                !(visitor is AsQueryableResultOperator.ISupportedByIQueryModelVistor))
                return;
        ...
        {
            if (!method.IsPublic)
                throw new ArgumentException("Method to invoke ('" + method.Name + "') must be a public method.",
                    "method");
            ...
            {
                var message = string.Format("Cannot call method '{0}' on input of type '{1}': {2}", method.Name,
                    input.GetType(), ex.Message);
