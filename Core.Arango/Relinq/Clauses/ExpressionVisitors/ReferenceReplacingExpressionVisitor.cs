...
namespace Core.Arango.Relinq.Clauses.ExpressionVisitors
{
    ...
    ///     <see cref="QuerySourceMapping" />.
    ///     This is used whenever references to query sources should be replaced by a transformation.
    /// </summary>
    internal sealed class ReferenceReplacingExpressionVisitor : RelinqExpressionVisitor
    {
        /// <summary>
        ///     Takes an expression and replaces all <see cref="QuerySourceReferenceExpression" /> instances, as defined by a given
        ...
        ///     <see cref="QuerySourceReferenceExpression" /> instances.
        /// </param>
        /// <param name="throwOnUnmappedReferences">
        ///     If <see langword="true" />, the visitor will throw an exception when
        ///     <see cref="QuerySourceReferenceExpression" /> not mapped in the <paramref name="querySourceMapping" /> is
        ///     encountered. If <see langword="false" />,
        ///     the visitor will ignore such expressions.
        /// </param>
        /// <returns>
        ///     An expression with its <see cref="QuerySourceReferenceExpression" /> instances replaced as defined by the
        ///     <paramref name="querySourceMapping" />.
        /// </returns>
        public static Expression ReplaceClauseReferences(Expression expression, QuerySourceMapping querySourceMapping,
            bool throwOnUnmappedReferences)
        {
            ArgumentUtility.CheckNotNull("expression", expression);
            ArgumentUtility.CheckNotNull("querySourceMapping", querySourceMapping);

            return new ReferenceReplacingExpressionVisitor(querySourceMapping, throwOnUnmappedReferences).Visit(
                expression);
        }
        ...
        private ReferenceReplacingExpressionVisitor(QuerySourceMapping querySourceMapping,
            bool throwOnUnmappedReferences)
        {
            ArgumentUtility.CheckNotNull("querySourceMapping", querySourceMapping);
        ...
        protected internal override Expression VisitQuerySourceReference(QuerySourceReferenceExpression expression)
        {
            ...
            if (_throwOnUnmappedReferences)
            {
                var message = "Cannot replace reference to clause '" + expression.ReferencedQuerySource.ItemName +
                              "', there is no mapped expression.";
        ...
        protected internal override Expression VisitSubQuery(SubQueryExpression expression)
        {
            ...
            expression.QueryModel.TransformExpressions(ex =>
                ReplaceClauseReferences(ex, _querySourceMapping, _throwOnUnmappedReferences));
            return expression;
        }
