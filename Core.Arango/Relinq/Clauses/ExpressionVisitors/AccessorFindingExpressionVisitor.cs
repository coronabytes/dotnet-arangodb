...
{
    /// <summary>
    ///     Constructs a <see cref="LambdaExpression" /> that is able to extract a specific simple expression from a complex
    ...
    ///         expression. This <see cref="AccessorFindingExpressionVisitor" /> will return a <see cref="LambdaExpression" />
    ...
    ///     </para>
    /// </example>
    internal sealed class AccessorFindingExpressionVisitor : RelinqExpressionVisitor
    {
        private readonly Stack<Expression> _accessorPathStack = new();
        ...
        /// <summary>
        ///     Constructs a <see cref="LambdaExpression" /> that is able to extract a specific simple
        ...
        /// </param>
        /// <remarks>
        ///     The <see cref="AccessorFindingExpressionVisitor" /> compares the <paramref name="searchedExpression" /> via
        ///     reference equality,
        ...
        ///     <see cref="NewExpression" /> or
        ///     <see cref="MemberInitExpression" />.
        /// </remarks>
        /// <returns>
        ///     A <see cref="LambdaExpression" /> acting as an accessor for the <paramref name="searchedExpression" /> when an
        ///     input matching
        ///     <paramref name="fullExpression" /> is given.
        /// </returns>
        public static LambdaExpression FindAccessorLambda(Expression searchedExpression, Expression fullExpression,
            ParameterExpression inputParameter)
        {
            ...
            if (inputParameter.Type != fullExpression.Type)
                throw new ArgumentException(
                    string.Format("The inputParameter's type '{0}' must match the fullExpression's type '{1}'.",
                        inputParameter.Type, fullExpression.Type),
                    "inputParameter");

            var visitor = new AccessorFindingExpressionVisitor(searchedExpression, inputParameter);
            visitor.Visit(fullExpression);

            if (visitor.AccessorPath != null) return visitor.AccessorPath;
        ...
        public override Expression Visit(Expression expression)
        {
            ...
            {
                var path = _accessorPathStack.Peek();
                AccessorPath = Expression.Lambda(path, _inputParameter);

                return expression;
            }

            if (expression is NewExpression || expression is MemberInitExpression || expression is UnaryExpression)
