...
namespace Core.Arango.Relinq.Clauses.ResultOperators
{
    ...
    ///     <see cref="AggregateFromSeedResultOperator" />.
    ///     <code>
    /// var result = (from s in Students
    ///              select s).Aggregate(0, (totalAge, s) => totalAge + s.Age);
    /// </code>
    /// </example>
    internal sealed class AggregateFromSeedResultOperator : ValueFromSequenceResultOperatorBase
    {
        ...
        /// <param name="func">
        ///     The aggregating function. This is a <see cref="LambdaExpression" /> taking a parameter that represents the value
        ///     accumulated so
        ...
        public AggregateFromSeedResultOperator(Expression seed, LambdaExpression func,
            LambdaExpression optionalResultSelector)
        {
            ...
            if (func.Type.GetTypeInfo().IsGenericTypeDefinition)
                throw new ArgumentException(
                    "Open generic delegates are not supported with AggregateFromSeedResultOperator", "func");
        ...
        /// <summary>
        ///     Gets or sets the aggregating function. This is a <see cref="LambdaExpression" /> taking a parameter that represents
        ...
        public LambdaExpression Func
        {
            ...
            set
            {
                ...
                if (value.Type.GetTypeInfo().IsGenericTypeDefinition)
                    throw new ArgumentException(
                        "Open generic delegates are not supported with AggregateFromSeedResultOperator", "value");
        ...
        /// <summary>
        ///     Gets or sets the seed of the accumulation. This is an <see cref="Expression" /> denoting the starting value of the
        ///     aggregation.
        /// </summary>
        ...
        /// <summary>
        ///     Gets or sets the result selector. This is a <see cref="LambdaExpression" /> applied after the aggregation to select
        ///     the final value.
        ...
        /// <summary>
        ///     Gets the constant value of the <see cref="Seed" /> property, assuming it is a <see cref="ConstantExpression" />. If
        ///     it is
        ...
        private StreamedValueInfo GetOutputDataInfo([NotNull] StreamedSequenceInfo sequenceInfo)
        {
            ...
            Assertion.DebugAssert(OptionalResultSelector == null ||
                                  OptionalResultSelector.Type.GetTypeInfo().IsGenericTypeDefinition == false);
            var resultTransformedType = OptionalResultSelector != null
                ? OptionalResultSelector.Type.GetTypeInfo().GenericTypeArguments[0]
                : null;
        ...
        {
            return value == null || value.Type.GetTypeInfo().IsGenericType &&
                value.Type.GetGenericTypeDefinition() == typeof(Func<,>);
        }
    }
}