...
{
    /// <summary>
    ///     Represents a <see cref="SequenceFromSequenceResultOperatorBase" /> that is executed on a sequence, returning a new
    ///     sequence with the same
    ///     item type as its result.
    /// </summary>
    internal abstract class SequenceTypePreservingResultOperatorBase : SequenceFromSequenceResultOperatorBase
    {
        ...
        protected StreamedSequenceInfo GetOutputDataInfo(StreamedSequenceInfo inputSequenceInfo)
        {
            ...
            return new StreamedSequenceInfo(typeof(IQueryable<>).MakeGenericType(inputSequenceInfo.ResultItemType),
                inputSequenceInfo.ItemExpression);
        }
    }
}