...
{
    /// <summary>
    ///     Provides a default implementation of <see cref="IQueryModelVisitor" /> which automatically visits child items. That
    ///     is, the default
    ///     implementation of <see cref="VisitQueryModel" /> automatically calls <c>Accept</c> on all clauses in the
    ///     <see cref="QueryModel" />
    ///     and the default implementation of <see cref="VisitOrderByClause" /> automatically calls
    ///     <see cref="Ordering.Accept" /> on the
    ///     <see cref="Ordering" /> instances in its <see cref="OrderByClause.Orderings" /> collection, and so on.
    /// </summary>
    /// <remarks>
    ///     This visitor is hardened against modifications performed on the visited <see cref="QueryModel" /> while the model
    ///     is currently being visited.
    ///     That is, if a the <see cref="QueryModel.BodyClauses" /> collection changes while a body clause (or a child item of
    ///     a body clause) is currently
    ///     being processed, the visitor will handle that gracefully. The same applies to
    ///     <see cref="QueryModel.ResultOperators" /> and
    ///     <see cref="OrderByClause.Orderings" />.
    /// </remarks>
    internal abstract class QueryModelVisitorBase : IQueryModelVisitor
    {
        ...
        public virtual void VisitJoinClause(JoinClause joinClause, QueryModel queryModel,
        ...
        public virtual void VisitOrdering(Ordering ordering, QueryModel queryModel, OrderByClause orderByClause,
            int index)
        {
        ...
        protected virtual void VisitOrderings(ObservableCollection<Ordering> orderings, QueryModel queryModel,
        ...
        protected virtual void VisitResultOperators(ObservableCollection<ResultOperatorBase> resultOperators,
